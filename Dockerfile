...
RUN comfy node install --exit-on-fail gguf@2.8.1
# The following unknown_registry custom nodes could not be automatically resolved (no aux_id provided):
# - Reroute, GetNode, SetNode, Label (rgthree), Note, Fast Groups Muter (rgthree), LoadImage, etc.
# Skipping git clone for unknown_registry because no GitHub repo (aux_id) entries were provided in the workflow JSON.
...
RUN comfy model download --url https://huggingface.co/lightx2v/Qwen-Image-Lightning/resolve/main/Qwen-Image-Lightning-4steps-V1.0-bf16.safetensors --relative-path models/loras --filename Qwen-Image-Lightning-4steps-V1.0-bf16.safetensors
...
RUN comfy model download --url https://huggingface.co/QuantStack/Qwen-Image-Edit-GGUF/resolve/main/Qwen_Image_Edit-Q5_1.gguf --relative-path models/diffusion_models --filename Qwen_Image_Edit-Q5_1.gguf
